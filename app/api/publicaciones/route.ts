...
async function ready(){return db.select({inmuebleId:inmInmuebles.id,codigo:inmInmuebles.codigo,tipo:inmInmuebles.tipo,referencia:inmInmuebles.referencia,posicion:inmPosiciones.numero,publicado:inmPublicaciones.publicado,fechaRegistro:inmPublicaciones.fechaRegistro}).from(inmPublicaciones).innerJoin(inmInmuebles,eq(inmInmuebles.id,inmPublicaciones.inmuebleId)).leftJoin(inmAsignacionesPosicion,and(eq(inmAsignacionesPosicion.inmuebleId,inmInmuebles.id),eq(inmAsignacionesPosicion.activa,true))).leftJoin(inmPosiciones,eq(inmPosiciones.id,inmAsignacionesPosicion.posicionId)).where(and(eq(inmInmuebles.estado,"activo"),eq(inmPublicaciones.publicado,false))).orderBy(desc(inmPublicaciones.fechaRegistro));}
export async function GET(){try{const rows=await approved();const listos=await ready();const publicadas=await published();return NextResponse.json({ok:true,pendientes:rows.map(map),listos,publicadas});}catch(error){console.error(error);return NextResponse.json({ok:false,error:"No se pudieron consultar los textos pendientes."},{status:500});}}
